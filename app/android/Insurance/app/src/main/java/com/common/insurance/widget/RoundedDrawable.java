...
public class RoundedDrawable extends Drawable {
    ...
    private void redrawBitmapForSquareCorners(Canvas canvas) {
        ...
        if (MathUtil.floatEqual(mCornerRadius, 0f, 3)) {
    ...
    public RoundedDrawable setCornerRadius(int corner, float radius) {
        if (!MathUtil.floatEqual(radius, 0f, 3) && !MathUtil.floatEqual(mCornerRadius, 0f, 3)
                && !MathUtil.floatEqual(mCornerRadius, radius, 3)) {
            throw new IllegalArgumentException("Multiple nonzero corner radii not yet supported.");
        }

        if (MathUtil.floatEqual(radius, 0f, 3)) {

...
import datetime
from myauth.models import MyUser, UserPhoto
...
def home(request):
    ...
    if request.city_id:
        ...
        if _count < 16:
            hot_user_proxy = MyUser.objects.filter(is_proxy=1, province_id=request.province_id).order_by("-ans_num",
                                                                                                         "uid")[:16]
...
def sitemap_index(request):
    def get_news_sitemap_lastmod(id):
        ...
        try:
            info = \
            News.objects.filter(nid__gte=id * 10000, nid__lt=(id + 1) * 10000).order_by("-addtime").values("addtime")[0]
            return info["addtime"]
        except:
            return None
    ...
    def get_product_sitemap_lastmod(id):
        id = int(id)
        try:
            info = Product.objects.filter(pid__gte=id * 10000, pid__lt=(id + 1) * 10000).order_by("-addtime").values(
                "addtime")[0]
            return info["addtime"]
        except:
            return None
    ...
    def get_ask_sitemap_lastmod(id):
        id = int(id)
        try:
            info = Ask.objects.filter(askid__gte=id * 10000, askid__lt=(id + 1) * 10000).order_by("-ask_time").values(
                "ask_time")[0]

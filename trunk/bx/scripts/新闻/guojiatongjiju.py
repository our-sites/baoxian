...
def list_handle(result):
    ...
    for i in doc(".cont_tit"):
        ...
        def detail_handle(result,title=title,addtime=addtime,fromurl=url):
            ...
            time.sleep(2)
        Spider(url,code="utf-8",response_handle=detail_handle,retry_times=3,timeout=30)
#for i in range(1,25):
Spider("http://www.stats.gov.cn/tjsj/zxfb/index.html",code="utf-8",response_handle=list_handle,timeout=30
       )
for i in range(1,2):
    Spider("http://www.stats.gov.cn/tjsj/zxfb/index_%s.html"%i,code="utf-8",response_handle=list_handle,proxy=False,timeout=30
    )
spider_join()
for i,j,k,l   in alldata:
    ...
    mgr.runOperation(''' insert ignore   into bx_consult(title, type, writer, `from`, addtime, content, status, type_cate)
                         VALUES (%s,4,"网络",%s,%s,%s,0,0)''',(i,l,int(time.time()),j))
...
def spider_init(poolsize,url_maxnum):
    print datetime.datetime.now(),"[爬虫]:初始化...."
...
class Spider(object):
    _url_buff=set()
    def __init__(self,url,code=None,data=None,cookie=None,handle=None,timeout=3,retry_times=30,retry_delta=3,proxy_opener=None,header=None):
        '''
            url   目标url
            data   post的数据
            timeout  超时时间
        ...
        global  _queue
        if url+str(data) not in Spider._url_buff and len(Spider._url_buff)<_url_max_num:
            Spider._url_buff.add(url+str(data))
            self.url=url
    ...
    def _go(self):
        ...
        for i in range(0,retry_times):
            try:
                ...
                if self.header:
                    for _t in self.header:
                        request.headers[_t]=self.header[_t]
                if self.cookie:
                ...
                print datetime.datetime.now(),"[爬虫]:%s Exception:%s"%(url,e)
                traceback.print_exc()
                time.sleep(retry_delta)
            else:
                print datetime.datetime.now(),"[爬虫]:%s Success!"%url
                break
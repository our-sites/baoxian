...
class UserPipeline(object):
    ...
    def _conditional_insert(self, tx, item):
        if item.get('toubao',0) :
            ...
            tx.execute("select aid from agent_caiji_baoxianla where url='%s'"%item['url'])
            result = tx.fetchone()
            if result:
                logging.log(logging.INFO,"Already stored in db: %s"%item["url"])
            else:
                ...
                try:
                    SQL = """INSERT INTO agent_caiji_baoxianla(aid,real_name,phone,QQ,certificate_code,bx_com,proxy_cid,proxy_workyear,city,city_id,province,province_id,url) 
                                                       VALUES(NULL,'%s',%s,%s,'%s','%s',%s,%s,'%s',%s,'%s',%s,'%s')"""
                    SQL=SQL%(item['name'],item["phone"],item["qq"],item['zige'],item['company'],cid,item['work'],item["city"],city_id,province,province_id,item['url'])
                    #print SQL,'~~~~~~~~~~',item.keys()
                    tx.execute(SQL)
                except Exception,e:
                    print e,item['url']

...
result=mgr.runQuery('''select nid,title,content,`from`  from bx_news  WHERE status=0   limit 10''',())

for _nid,title,content,_from  in  result:
    if not content:
        mgr.runOperation('''delete from  bx_news   where nid=%s''',
                         (_nid,))
    else:
        ...
        tags=",".join ([i  for i,j in   pseg.cut(title) if j!="v"][:5])
        print keywords
        print zhaiyao
        # for _ in  doc("p"):
        ...
        doc("script").remove()
        content= doc.html()
        mgr.runOperation('''update bx_news  set content=%s,status=3,keywords=%s ,abstract=%s,tags=%s  where nid=%s''',(content,keywords,zhaiyao,tags,_nid))
        print _nid
